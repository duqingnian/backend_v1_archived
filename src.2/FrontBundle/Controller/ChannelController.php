...
class ChannelController extends \AppBundle\Controller\BaseController 
{
	...
	public function _create($request)
    {
		...
		{
			//代收appid
			$payin_appid  = md5(date('YmdHis').$channel->getId().md5(microtime()).rand(123456,999999));
			$payin_secret = $this->authcode('APPID'.$payin_appid);
			
			//代付appid
			$payout_appid  = md5(date('YmdHis').$channel->getId().md5(microtime()).rand(123456,999999).$payin_appid);
