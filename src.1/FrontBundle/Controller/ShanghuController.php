...
class ShanghuController extends \AppBundle\Controller\BaseController 
{
	...
	public function _create($request)
    {
		...
		if('true' == $is_active) $is_active = 1;
		if('false' == $is_active) $is_active = 0;
		if('true' == $is_test) $is_test = 1;
		if('false' == $is_test) $is_test = 0;
		...
		{
			//代收appid
			$payin_appid  = md5(date('YmdHis').$shanghu->getId().md5(microtime()).rand(123456,999999));
			$payin_secret = $this->authcode('APPID'.$payin_appid);
			
			//代付appid
			$payout_appid  = md5(date('YmdHis').$shanghu->getId().md5(microtime()).rand(123456,999999).$payin_appid);
			$payout_secret = $this->authcode('APPID'.$payout_appid);
			
			$config = new \AppBundle\Entity\ShanghuConfig();
			$config->setMasterId($shanghu->getId());
			$config->setPayinAppid(substr($payin_appid,0,8));
			$config->setPayinSecret(substr($payin_secret,0,32));
			$config->setPayoutAppid(substr($payout_appid,0,8));
			$config->setPayoutSecret(substr($payout_secret,0,32));
			$this->save($config);
	...
	private function _update($request) 
	{
		...
		$config->setPayinPct($in_pct);
		$config->setPayinSigleFee($in_sigle_fee);
		$config->setPayoutPct($out_pct);
		$config->setPayoutSigleFee($out_sigle_fee);
